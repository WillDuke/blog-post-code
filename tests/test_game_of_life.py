...
def test_array_size(starting_grid, neighbors, second_grid):
    ...
    assert all(
        [arr.shape == (10, 10) for arr in (starting_grid, neighbors, second_grid)]
    )


def test_first_rule(starting_grid, neighbors, second_grid):
    """rule 1: any live cell with fewer than two neighbors dies"""
    assert not np.any(second_grid[np.bool_(starting_grid & (neighbors < 2))]), (
        second_grid,
        neighbors,
    )
...
def test_second_rule(starting_grid, neighbors, second_grid):
    ...
    assert np.all(
        second_grid[np.bool_(starting_grid & ((neighbors == 2) | (neighbors == 3)))]
    )
